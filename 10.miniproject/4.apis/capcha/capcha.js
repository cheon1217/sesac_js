...
async function getCaptchaImage(key) {
    ...
    try {
        ...
        console.log(`캡차 이미지가 ${path}에 적용되었습니다.`);
    } catch (error) {
...
async function verifyCaptcha(key, userInput) {
    ...
    try {
        ...
        console.error("캡챠 검증 오류: ", error.response.data);
    }
}
...
(async () => {
    ...
    if (key) {
        ...
        console.log("이미지를 확인하고 캡챠 텍스트를 입력하세요.");
        const userInput = await getUserInput("캡챠 입력: ");
        const isValid = await verifyCaptcha(key, userInput);
        console.log(`캡챠 검증 결과: ${isValid}`);
    }
})();